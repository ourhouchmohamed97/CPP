...
int Span::shortestSpan() {
    if (integers.size() < 2)
        throw std::exception();  // Or better: std::logic_error
    // ... implementation
}
```
...
**Used with:**
- STL algorithms for custom behavior
- Sorting with custom comparisons
- Finding with custom predicates

### Lambda Expressions (C++11)

**Modern alternative to functors:**
...
**In C++98:**
- Must use function pointers or functors
- Lambdas not available
...
typename Container::value_type    // Element type
typename Container::size_type     // Size type
```
...
- MutantStack enables this for stack!

---

## Comparison: Container Adaptors vs Full Containers

| Feature | std::vector/list | std::stack | MutantStack |
|---------|------------------|------------|-------------|
| **Random Access** | ✅ (vector) | ❌ | ❌ |
| **Iterators** | ✅ | ❌ | ✅ |
| **push_back()** | ✅ | Via push() | Via push() |
| **top()** | Via back() | ✅ | ✅ |
| **Access Middle** | ✅ | ❌ | Via iterator |
| **STL Algorithms** | ✅ | ❌ | ✅ |
| **Interface** | Full container | Restricted | Restricted + iterators |
...
std::cout << *it;  // Crashes if not found!
...
// ❌ Dangerous
int* ptr = &vec[0];
vec.push_back(42);  // May reallocate, ptr now invalid!
std::cout << *ptr;  // Undefined behavior

// ✅ Correct
vec.reserve(size_needed);  // Pre-allocate
int* ptr = &vec[0];
vec.push_back(42);  // Won't reallocate if within capacity
```

### 4. Not Checking Size
```cpp
// ❌ Wrong
int span = vec[1] - vec[0];  // Crashes if size < 2
